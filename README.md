...
-F2 Bootloader
--Bootloader demo for the STM32F2 based units (older units)

-F4 Bootloader
--Bootloader demo for the STM32F4 based units (current production)
...
-T2 AppPack
--This is utilized to sign the standard .hex output files to a .bin that the demo bootloader will recognize.

-F2 Demos

T2_F4_VCP_Demo
--Virtual COM port demo that allows the user to talk with the internal modem via AT commands through
the USB port instead of the DB9.
