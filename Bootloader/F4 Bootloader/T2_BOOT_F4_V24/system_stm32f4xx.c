/**
  ******************************************************************************
  * @file    system_stm32f4xx.c ##### MODIFIED FOR T2 ####
...
/******************************************************************************/
  ...
  {
		if ((RCC->CR & RCC_CR_HSIRDY) != RESET) // Should Already be ticking at reset
			Status = 2; // HSI Usable
		else
			Status = 0; // How are we even running this?
  }

  if (Status) // HSE or HSI Running
  {
		...
		else // Fall back to HSI, some PLL runs and SDIOCLK is viable
		{
			/* Configure the main PLL - PLL_M hard coded to 16 MHz for HSI */
			RCC->PLLCFGR = 16 | (PLL_N << 6) | (((PLL_P >> 1) -1) << 16) | (RCC_PLLCFGR_PLLSRC_HSI) | (PLL_Q << 24);
		}
