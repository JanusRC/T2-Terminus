...
#define CPUSPEED 120000000
//#define CPUSPEED 320000000
...
void ClockDiagnostic(void)
{
  ...
  PWR_BackupAccessCmd(ENABLE); /* Allow access to BKP Domain */
	...
	// LSE startup time  2s not untypical
...
#else
{
	unsigned int dwell, stop, start = *((volatile unsigned int *)0xE0001004); // DWT_CYCCNT
	
	i = 0;

 	while((i++ < 30000) && (!(RCC->BDCR & RCC_BDCR_LSERDY) || !(RCC->CSR & RCC_CSR_LSIRDY))) /* Wait till LSI/LSE is ready */
		TimingDelay(RCC_ClockFreq.SYSCLK_Frequency / 10000);

	stop = *((volatile unsigned int *)0xE0001004);
	dwell = stop - start;
	printf("Dwell %10d - %5d ms\n", dwell, dwell / (RCC_ClockFreq.SYSCLK_Frequency / 1000)); // Time spent initially starting

#define LSE_ITER 5
	
  if ((RCC_GetFlagStatus(RCC_FLAG_LSERDY) != RESET) &&	// External 32.768 KHz running
			(RCC_GetFlagStatus(RCC_FLAG_HSERDY) != RESET))		// External 16 MHz crystal must be running otherwise test pointless
	{
		i = LSE_ITER;
		...
		do{
			...
			stop = *((volatile unsigned int *)0xE0001004);
			dwell = stop - start;
			printf("      %10d - %5d ms  %5d\n", dwell, dwell / (RCC_ClockFreq.SYSCLK_Frequency / 1000), lsefreq);

			if ((dwell / RCC_ClockFreq.SYSCLK_Frequency) >= 5) // 5 Second time out
				break;
			
			if (lsefreq == 32768) i--; else i = LSE_ITER;
		}while((lsefreq != 32768) || i);
	}
}
#endif
  ...
  if (RCC_GetFlagStatus(RCC_FLAG_LSERDY) != RESET)
	...
	if ((lsefreq < 32767) || (lsefreq > 32769)) // Should be on the money
		OutputDiagnostic++;
...
//	  puts("PLL I2S Not Ready");
		...
		if ((lsefreq < 32767) || (lsefreq > 32769)) // Should be on the money, suprised if not 32768
...
int ApplicationFirmwareCheck(void)
{
	...
	if (*((unsigned long *)(APPLICATION_BASE + 0x184)) == 0xDEADBEEF) // Unchecked Debug Application
	{
...
void USART6_Configuration(UART *Uart)
{
  ...
  GPIO_SetBits(GPIOC, GPIO_InitStructure.GPIO_Pin); // PC.1 Plug_in terminal power supply is enabled
...
void GPIO_Configuration(void)
{
  ...
  GPIO_ResetBits(GPIOC, GPIO_Pin_1 | GPIO_Pin_3); // PC.1 Plug_in terminal power supply is disabled
}
...
#ifdef REV2
	printf("\n\n\nJanus Remote Communications - TERMINUS 2 R1 - Boot Loader\nFW: %s %s (v2.1)\n\n", __DATE__, __TIME__);
#else // REV1
	printf("\n\n\nJanus Remote Communications - TERMINUS 2 R0 - Boot Loader\nFW: %s %s (v2.1)\n\n", __DATE__, __TIME__);
#endif
